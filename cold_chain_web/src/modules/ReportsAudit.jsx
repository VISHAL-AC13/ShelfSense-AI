...
export default function ReportsAudit({ shipments }) {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text("Food Safety & Quality Assurance Document // Certified Trip Report", 14, 28);
    ...
    autoTable(doc, {
      startY: nextY + 5,
      head: [["Priority", "Recommended Action", "Quality Diagnostic Rationale"]],
      body: recRows,
      theme: "striped",
    ...
    // Footer & Digital Verification Stamp
    const pageHeight = doc.internal.pageSize.height || doc.internal.pageSize.getHeight();
    doc.setFontSize(8);
    doc.setTextColor(100, 116, 139);
    doc.text(`Generated by ShelfSense QA Portal // Certified Trip Report // Timestamp: ${new Date().toISOString()}`, 14, pageHeight - 12);
    doc.text(`Digital Verification Checksum: QC-VERIFY-TN-${shipment.shipment_id}-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}`, 14, pageHeight - 7);
